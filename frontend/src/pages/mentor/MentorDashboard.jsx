...
const MentorDashboard = () => {
  ...
  return (
    <div className="w-full space-y-8 px-4 md:px-8 pb-12">
      ...
      <header
        className="relative w-full rounded-[0.5rem] bg-[#0c0c0c] border border-white/10 overflow-hidden animate-evolve-in flex flex-col md:flex-row items-center justify-between p-8 md:p-12 mb-4 shadow-2xl"
      ...
      >
        ...
        <div className="relative z-10 w-full md:w-3/5 space-y-6">
          <div>
            <p className="text-[#e87315]/80 text-[11px] sm:text-xs font-black tracking-[0.3em] uppercase mb-3 sm:mb-4">Ecosystem Overview</p>
            ...
            <p className="text-white/60 mt-4 text-sm sm:text-base font-medium max-w-md leading-relaxed">
              You are currently guiding <span className="text-white/90 font-black">{menteeCount}</span> {menteeCount === 1 ? 'mentee' : 'mentees'} with <span className="text-white/90 font-black">{pending.length}</span> pending {pending.length === 1 ? 'inquiry' : 'inquiries'} today.
            </p>
          </div>
          ...
          {pending.length > 0 ? (
            ...
            <div className="inline-flex items-center gap-3 px-4 sm:px-5 py-2.5 sm:py-3 bg-white/[0.05] border border-white/20 backdrop-blur-md shadow-lg">
              <div className="relative flex items-center justify-center w-3 h-3">
                <span className="absolute inline-flex w-full h-full rounded-full bg-emerald-500 opacity-75 animate-ping" />
                <span className="relative inline-flex w-2 h-2 rounded-full bg-emerald-500" />
              </div>
              <span className="text-[11px] sm:text-xs font-bold text-white/60">
                Status: <span className="text-white/90">{user?.mentorStatus || 'Accepting Mentees'}</span>
              </span>
            </div>
          )}
        </div>
        ...
        <div className="relative z-10 hidden md:flex w-full md:w-2/5 justify-end mt-8 md:mt-0" style={{ perspective: '1000px' }}>
          ...
          <div className="relative w-64 h-52 [transform:rotateX(10deg)_rotateY(-15deg)] hover:[transform:rotateX(5deg)_rotateY(-5deg)_translateY(-5px)] transition-transform duration-700 ease-out group cursor-default">
            <div className="absolute inset-0 bg-gradient-to-bl from-[#1a1a1a]/90 to-[#0c0c0c]/90 border border-white/20 group-hover:border-[#e87315]/40 rounded-3xl shadow-2xl p-6 flex flex-col justify-between overflow-hidden backdrop-blur-md transition-colors duration-500">
              ...
              <div className="flex justify-between items-start relative z-10">
                <div className="w-12 h-12 rounded-xl bg-[#e87315]/10 flex items-center justify-center border border-[#e87315]/20 group-hover:bg-[#e87315]/20 transition-colors">
                  <Users size={24} className="text-[#e87315]" />
                </div>
                <div className="text-right">
                  <p className="text-[10px] sm:text-[11px] font-black text-[#e87315]/80 tracking-widest uppercase">Mentees</p>
                  <p className="text-2xl font-black text-white/90 leading-none mt-1.5">{menteeCount}</p>
                </div>
              </div>
              ...
              <div className="space-y-3.5 w-full relative z-10">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] sm:text-[11px] font-bold text-white/60 uppercase tracking-widest">Network</span>
                  </div>
                  <span className="text-[10px] sm:text-[11px] font-black text-white/90">{network.length}</span>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
