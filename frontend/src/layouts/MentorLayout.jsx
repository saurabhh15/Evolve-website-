...
import {
  Menu, X, Bell, Users, MessageCircle,
  LayoutDashboard, Calendar, Search,
  SquareArrowRightEnter, LogOut
} from 'lucide-react';
...
const MentorLayout = ({ children }) => {
  ...
  useEffect(() => {
    document.body.style.overflow = connectionsOpen ? 'hidden' : '';
  ...
  useEffect(() => {
    const openHandler = () => { setConnectionsOpen(true); fetchConnections(); };
    const clearHandler = () => setUnreadMessages(0);
    window.addEventListener('openConnections', openHandler);
    window.addEventListener('clearUnreadMessages', clearHandler);
    return () => {
      window.removeEventListener('openConnections', openHandler);
      window.removeEventListener('clearUnreadMessages', clearHandler);
    };
  }, []);
  ...
  const fetchConnections = async () => {
    try {
      ...
      const res = await connectionAPI.getNetwork();
  ...
  const handleRequestAction = async (connectionId, status) => {
    try {
      ...
      setNotifications(prev => ({ ...prev, requests: prev.requests.filter(r => r._id !== connectionId) }));
  ...
  const handleMarkAsRead = async (notifId) => {
    try {
      ...
      setNotifications(prev => ({ ...prev, notifs: prev.notifs.map(n => n._id === notifId ? { ...n, read: true } : n) }));
      setUnreadCount(prev => Math.max(prev - 1, 0));
    } catch (err) {
  ...
  const navItems = [
    ...
    { name: 'Mentor Hub', path: '/dashboard/MentorHub', icon: Calendar },
    { name: 'Inbox', path: '/dashboard/messages', icon: MessageCircle },
  ...
  return (
    <div className="min-h-[100dvh] bg-[#080808] text-white relative flex flex-col font-sans overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-[100] bg-[#080808]/90 backdrop-blur-xl border-b border-white/[0.03] w-full">
        <div className="max-w-[1400px] mx-auto px-3 sm:px-4 md:px-8 h-16 sm:h-20 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <div className="w-8 h-8 sm:w-12 sm:h-11 rounded-xl flex">
          ...
          {/* Desktop Nav */}
          <div className="hidden md:flex items-center gap-0.5 lg:gap-1 bg-[#080808] p-1 border border-white/5 relative overflow-hidden flex-shrink min-w-0">
            <div className="absolute inset-0 opacity-[0.03] pointer-events-none"
              style={{ backgroundImage: 'linear-gradient(#fff 1px, transparent 1px), linear-gradient(90deg, #fff 1px, transparent 1px)', backgroundSize: '20px 20px' }} />
            {navItems.map((item) => {
              const isActive = location.pathname === item.path || (item.name === 'Home' && location.pathname === '/dashboard');
              return (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`relative flex items-center gap-1.5 lg:gap-3 px-3 lg:px-5 py-2.5 lg:py-3 transition-all duration-500 group whitespace-nowrap ${
                    isActive ? 'bg-white text-black shadow-[4px_4px_0px_rgba(232,115,21,1)]' : 'text-white/40 hover:text-white hover:bg-white/[0.02]'
                  }`}
                >
                  {isActive && <div className="absolute top-0 left-0 w-1.5 h-1.5 border-t border-l border-black/30" />}
                  <item.icon size={13} className={`shrink-0 transition-colors duration-500 ${isActive ? 'text-black' : 'text-[#e87315]/50 group-hover:text-[#e87315]'}`} strokeWidth={isActive ? 3 : 2} />
                  <span className="text-[9px] lg:text-[10px] font-black tracking-[0.2em] lg:tracking-[0.25em] uppercase italic">{item.name}</span>
                  {item.name === 'Inbox' && unreadMessages > 0 && (
                    <span className={`w-1.5 h-1.5 rounded-none rotate-45 animate-pulse shrink-0 ${isActive ? 'bg-black' : 'bg-[#e87315]'}`} />
                  )}
                  {!isActive && <div className="absolute bottom-0 left-1/2 w-0 h-[1px] bg-[#e87315] group-hover:w-1/2 group-hover:left-1/4 transition-all duration-500" />}
                </Link>
              );
            })}
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-4 shrink-0">
            ...
            <button
              ...
              className="group relative p-2 sm:p-3 bg-transparent transition-all duration-300 hidden sm:block"
            >
              ...
              <div className="relative z-10">
                <Users size={16} strokeWidth={2} className="text-white/20 group-hover:text-white transition-all duration-300 group-hover:scale-110" />
              </div>
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="group relative p-2 sm:p-3 bg-transparent transition-all duration-300"
              >
                ...
                <div className="relative z-10">
                  <Bell size={16} strokeWidth={2} className="text-white/20 group-hover:text-white group-hover:rotate-[15deg] transition-all duration-500" />
                </div>
                {unreadCount > 0 && (
                ...
                <div className="absolute right-0 top-12 sm:top-14 w-[calc(100vw-2rem)] sm:w-80 max-w-sm bg-[#101010] border border-white/[0.06] rounded-2xl shadow-2xl z-[110] overflow-hidden">
                  <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-white/5 bg-white/[0.01]">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-[11px] font-black text-white uppercase tracking-[0.3em] italic">Notifications</h3>
                    </div>
                    <div className="flex items-center gap-3">
                      {unreadCount > 0 && (
                        <span className="px-2 py-0.5 bg-white text-black text-[9px] font-black">{unreadCount.toString().padStart(2, '0')}</span>
                      )}
                      {notifications.notifs?.some(n => !n.read) && (
                        <button
                          onClick={async () => {
                            await notificationAPI.markAllAsRead();
                            setNotifications(prev => ({ ...prev, notifs: prev.notifs.map(n => ({ ...n, read: true })) }));
                            setUnreadCount(notifications.requests?.length || 0);
                          }}
                          className="text-[9px] font-black text-white/30 hover:text-[#e87315] transition-all uppercase tracking-[0.15em]"
                        >
                          Mark All Read
                        </button>
                      )}
                    ...
                    {notifLoading ? (
                      ...
                      <>
                        {notifications.requests?.map((req) => (
                          <div key={req._id} className="group px-4 sm:px-6 py-4 sm:py-5 border-b border-white/5 hover:bg-white/[0.01] transition-all relative">
                            ...
                            <div className="flex items-start gap-3 sm:gap-4">
                              <div className="relative flex-shrink-0">
                                <img
                                  ...
                                  className="w-9 h-9 sm:w-10 sm:h-10 object-cover border border-white/10 grayscale group-hover:grayscale-0 transition-all duration-500"
                                  alt={req.from?.name}
                                />
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2">
                                  <p className="text-[11px] font-black text-white uppercase tracking-wider truncate">{req.from?.name}</p>
                                  <span className="text-[8px] font-bold text-[#e87315]/40 uppercase italic shrink-0">Pending</span>
                                </div>
                                ...
                                <div className="flex gap-2 mt-3">
                                  <button onClick={() => handleRequestAction(req._id, 'accepted')} className="flex-1 py-1.5 bg-[#e87315] hover:bg-white text-black text-[9px] font-black uppercase tracking-[0.2em] transition-all">Accept</button>
                                  <button onClick={() => handleRequestAction(req._id, 'rejected')} className="flex-1 py-1.5 bg-transparent border border-white/10 hover:border-red-500/50 text-white/20 hover:text-red-500 text-[9px] font-black uppercase tracking-[0.2em] transition-all">Decline</button>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        {notifications.notifs?.map((notif) => (
                          <div
                            key={notif._id}
                            onClick={() => { handleMarkAsRead(notif._id); if (notif.project) navigate(`/dashboard/project/${notif.project._id}`); setNotifOpen(false); }}
                            className={`px-4 sm:px-6 py-4 sm:py-5 border-b border-white/5 hover:bg-white/[0.02] transition-all cursor-pointer group relative ${!notif.read ? 'bg-white/[0.01]' : ''}`}
                          >
                            ...
                            <div className="flex items-start gap-3 sm:gap-4">
                              <img
                                ...
                                className={`w-8 h-8 sm:w-9 sm:h-9 object-cover border shrink-0 ${notif.read ? 'border-white/5 opacity-40' : 'border-white/20'} grayscale group-hover:grayscale-0 transition-all`}
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {notif.comment?.content && (
                                  <div className="mt-2 pl-3 border-l-2 border-white/5 group-hover:border-[#e87315]/20 transition-colors">
                                    <p className="text-[10px] text-white/30 italic line-clamp-1">"{notif.comment.content}"</p>
                                  </div>
                                )}
                              </div>
                            </div>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center gap-3">
                      <span className="text-white/10 group-hover:text-[#e87315]/40 transition-colors">[</span>
                      <p className="text-[9px] font-black text-white/40 group-hover:text-[#e87315] uppercase tracking-[0.4em] transition-colors">View all notifications</p>
                      <span className="text-white/10 group-hover:text-[#e87315]/40 transition-colors">]</span>
                    </div>
            ...
            <div className="hidden lg:block h-8 w-px bg-white/[0.04] mx-1" />

            {/* Profile */}
            <Link
              to="/dashboard/myprofile"
              className="hidden sm:flex items-center gap-2 lg:gap-4 p-1 bg-transparent group border-l border-white/5 hover:border-[#e87315] transition-all duration-500"
            >
              <div className="relative w-8 h-8 sm:w-10 sm:h-10 overflow-hidden border border-white/10 group-hover:border-[#e87315]/50 transition-colors duration-500 shrink-0">
                {user?.profileImage ? (
                  <img
                  ...
                  <div className="w-full h-full bg-[#080808] flex items-center justify-center">
                    <span className="text-[#e87315] font-black text-sm">{user?.name?.charAt(0)?.toUpperCase() || 'U'}</span>
                  </div>
                )}
              ...
              <div className="text-left hidden lg:block pr-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] font-black text-white uppercase tracking-wider group-hover:text-[#e87315] transition-colors truncate max-w-[120px]">
                    {user?.name || 'User'}
                  </p>
                </div>
        ...
        {/* Mobile dropdown */}
        <div className={`md:hidden absolute top-full left-0 w-full bg-[#080808] border-b border-white/5 overflow-hidden transition-all duration-500 ease-in-out z-50 ${isMobileMenuOpen ? 'max-h-[85vh] opacity-100' : 'max-h-0 opacity-0'}`}>
          ...
          <div className="relative p-3 flex flex-col gap-1 bg-[#080808]">
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3 mb-1 border border-white/5 hover:border-[#e87315]/30 transition-colors bg-[#0a0a0a]"
            >
              <div className="w-9 h-9 overflow-hidden border border-white/10 shrink-0">
                {user?.profileImage ? (
                  <img src={user.profileImage} className="w-full h-full object-cover" alt={user?.name} onError={(e) => { e.target.src = `https://ui-avatars.com/api/?background=111111&color=e87315&size=100&name=${encodeURIComponent(user.name || 'U')}&bold=true`; }} />
                ) : (
                  <div className="w-full h-full bg-[#080808] flex items-center justify-center">
                    <span className="text-[#e87315] font-black text-sm">{user?.name?.charAt(0)?.toUpperCase() || 'U'}</span>
                  </div>
              ...
              <div>
                <p className="text-[11px] font-black text-white uppercase tracking-wider">{user?.name || 'User'}</p>
                <p className="text-[9px] text-[#e87315]/60 uppercase tracking-widest italic">{user?.role || 'Mentor'}</p>
              </div>
            </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  to={item.path}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`relative flex items-center gap-4 px-5 py-3.5 transition-all duration-300 group ${
                    isActive ? 'bg-white text-black' : 'text-white/40 hover:text-white hover:bg-white/[0.02]'
                  }`}
                >
                  {isActive && <div className="absolute left-0 top-0 w-1 h-full bg-[#e87315]" />}
                  <item.icon size={15} className={`shrink-0 ${isActive ? 'text-black' : 'text-[#e87315]/40 group-hover:text-[#e87315]'}`} strokeWidth={isActive ? 3 : 2} />
                  <span className="text-[11px] font-black tracking-[0.3em] uppercase italic">{item.name}</span>
                  {item.name === 'Inbox' && unreadMessages > 0 && (
                    <div className="ml-auto flex items-center gap-2">
                      <span className="text-[9px] font-mono text-[#e87315] opacity-60">NEW</span>
            ...
            <div className="mt-1 h-px w-full bg-gradient-to-r from-transparent via-white/5 to-transparent" />
            <button
              onClick={() => { setConnectionsOpen(true); fetchConnections(); setIsMobileMenuOpen(false); }}
              className="flex items-center gap-4 px-5 py-3.5 text-white/40 hover:text-white hover:bg-white/[0.02] transition-all group"
            >
              <Users size={15} className="text-[#e87315]/40 group-hover:text-[#e87315] shrink-0" strokeWidth={2} />
              <span className="text-[11px] font-black tracking-[0.3em] uppercase italic">Network</span>
            </button>
            <button
              onClick={() => { logout(); setIsMobileMenuOpen(false); }}
              className="flex items-center gap-4 px-5 py-3.5 text-white/40 hover:text-red-400 hover:bg-white/[0.02] transition-all group"
            >
              <LogOut size={15} className="text-white/20 group-hover:text-red-400 shrink-0" strokeWidth={2} />
              <span className="text-[11px] font-black tracking-[0.3em] uppercase italic">Sign Out</span>
            </button>
          </div>
        </div>
      </nav>

      {/* Main content - flex-1 pushes footer/content correctly */}
      ...
      {connectionsOpen && (
        <div
          className="fixed inset-0 bg-[#050505]/90 backdrop-blur-md z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4"
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/5 relative z-10 bg-[#080808]">
              ...
              <button onClick={() => setConnectionsOpen(false)} className="group relative p-2 border border-white/5 hover:border-[#e87315]/50 transition-colors">
            ...
            <div className="px-6 py-4 border-b border-white/5 bg-[#0a0a0a] space-y-3 relative z-10">
              <div className="relative">
                <input
                  ...
                  className="w-full pl-4 pr-10 py-2.5 bg-transparent border border-white/5 hover:border-white/10 focus:border-[#e87315]/40 text-white text-[11px] font-bold uppercase tracking-wider focus:outline-none transition-all placeholder:text-white/10"
                />
                ...
                {['All', 'Student', 'Mentor', 'Investor'].map(filter => (
                  <button
                    ...
                    className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest transition-all ${
                      connectionFilter === filter ? 'bg-[#e87315] text-black' : 'bg-transparent text-white/30 hover:text-white border border-white/5'
                    }`}
                  >
                    {filter}
                  </button>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 relative z-10 bg-[#080808]">
              {connectionsLoading ? (
                ...
                filteredConnections.map(connection => (
                  <div
                    key={connection._id}
                    onClick={() => { navigate(connection.role?.toLowerCase() === 'mentor' ? `/dashboard/mentor/${connection._id}` : `/dashboard/user/${connection._id}`); setConnectionsOpen(false); }}
                    className="flex items-center gap-4 p-3 sm:p-4 hover:bg-white/[0.02] cursor-pointer transition-all group border border-transparent hover:border-white/5 relative overflow-hidden"
                  >
                    <div className="absolute left-0 top-0 w-[1px] h-0 group-hover:h-full bg-[#e87315] transition-all duration-500" />
                    <img
                      src={connection.profileImage}
                      onError={(e) => { e.target.src = `https://ui-avatars.com/api/?background=080808&color=e87315&size=100&name=${connection.name}&bold=true`; }}
                      className="w-10 h-10 sm:w-11 sm:h-11 object-cover border border-white/10 grayscale group-hover:grayscale-0 transition-all duration-500 shrink-0"
                      alt={connection.name}
                    />
                    <div className="flex-1 min-w-0">
