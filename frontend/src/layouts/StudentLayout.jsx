...
import {
  ...
  LayoutDashboard, Rocket, SquareChartGantt,
  GraduationCap, Search, LogOut
} from 'lucide-react';
...
const StudentLayout = ({ children }) => {
  ...
  // Lock body scroll when connections modal is open
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        setUnreadMessages(unreadMsgRes.data.unreadCount || 0);
      } catch (err) {
  ...
  // Poll unread messages every 10s
  useEffect(() => {
    const poll = async () => {
      try {
  ...
  const navItems = [
    ...
    { name: 'My Project', path: '/dashboard/project', icon: Rocket },
    { name: 'Explore', path: '/dashboard/explore', icon: SquareChartGantt },
    { name: 'Mentors', path: '/dashboard/mentors', icon: GraduationCap },
      ...
      <nav className="sticky top-0 z-50 bg-[#080808]/80 backdrop-blur-xl border-b border-white/[0.03]">
        <div className="max-w-[1400px] mx-auto px-3 sm:px-4 md:px-8 h-16 sm:h-20 flex items-center justify-between gap-2">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <item.icon
                    size={13}
                    className={`transition-colors duration-500 shrink-0 ${isActive ? 'text-black' : 'text-[#e87315]/50 group-hover:text-[#e87315]'}`}
                    strokeWidth={isActive ? 3 : 2}
                  />
                  <span className="text-[9px] lg:text-[10px] font-black tracking-[0.2em] lg:tracking-[0.25em] uppercase italic">
                    {item.name}
                  </span>
                  {item.name === 'Inbox' && unreadMessages > 0 && (
                    <span className={`w-1 h-1 rounded-none rotate-45 animate-pulse shrink-0 ${isActive ? 'bg-black' : 'bg-[#e87315]'}`} />
                  )}
                  {!isActive && (
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-4 shrink-0">
                ...
                <div className="absolute right-0 top-12 sm:top-14 w-[calc(100vw-2rem)] sm:w-80 max-w-sm bg-[#101010] border border-white/[0.06] rounded-2xl shadow-2xl z-50 overflow-hidden">
                  ...
                  <div className="max-h-72 sm:max-h-96 overflow-y-auto scrollbar-hide bg-[#080808]">
                    {notifLoading ? (
                      <div className="flex flex-col items-center justify-center py-16 gap-4">
                        ...
                        <span className="text-[9px] font-black text-[#e87315] uppercase tracking-[0.3em] animate-pulse">Syncing</span>
                      </div>
                    ) : (
                      <>
                            ...
                            <div className="flex items-start gap-3 sm:gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <span className="text-[8px] font-bold text-[#e87315]/40 uppercase tracking-tighter italic shrink-0">Pending</span>
                                </div>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center gap-3">
                      <span className="text-white/10 group-hover:text-[#e87315]/40 transition-colors duration-500">[</span>
                      <p className="text-[9px] font-black text-white/40 group-hover:text-[#e87315] uppercase tracking-[0.4em] transition-colors duration-500">View all notifications</p>
                      <span className="text-white/10 group-hover:text-[#e87315]/40 transition-colors duration-500">]</span>
                    </div>
            ...
            <div className="hidden lg:block h-8 w-px bg-white/[0.04]" />
          ...
          <div className="relative p-3 flex flex-col gap-1">
            ...
            >
              ...
              <div>
                ...
                <p className="text-[9px] text-[#e87315]/60 uppercase tracking-widest italic">{user?.role || 'Member'}</p>
              </div>
            </Link>
