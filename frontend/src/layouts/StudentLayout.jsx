...
import { Menu, X, Bell, User, Users, MessageCircle, LayoutDashboard, Rocket, SquareChartGantt, GraduationCap, Search } from 'lucide-react';
...
const StudentLayout = ({ children, setActiveTab, activeTab }) => {
  ...
  const handleRequestAction = async (connectionId, status) => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('connection_actioned', { detail: { id: connectionId } }));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const onConnectionRequest = (e) => {
      ...
      setPendingConnections(prev => {  // ← add this
  ...
  const navItems = [
    ...
    { name: 'Explore Projects', path: '/dashboard/explore', icon: SquareChartGantt },
              ...
              return (
                ...
                >
                  ...
                  {item.name === 'Inbox' && unreadMessages > 0 && (
                    <span className={`ml-1 w-1 h-1 rounded-none rotate-45 animate-pulse ${isActive ? 'bg-black' : 'bg-[#e87315]'}`} />
                  )}
                        ...
                        <div className="flex flex-col gap-1.5 shrink-0">
                          <button
                            onClick={async (e) => {
                              ...
                              window.dispatchEvent(new CustomEvent('connection_actioned', { detail: { id: conn._id } }));
                            }}
